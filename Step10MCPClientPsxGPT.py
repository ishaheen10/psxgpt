...
async def check_server_health() -> Dict[str, Any]:
    """Check the health of the MCP server using the new health check tool"""
    try:
        log.info("🏥 Checking server health...")
        result = await call_mcp_server("psx_health_check", {})
        
        if "error" in result:
            log.warning(f"⚠️ Health check returned error: {result['error']}")
            return result
        ...
        companies_available = result.get("companies_available", 0)
        
        if status == "healthy":
            log.info(f"✅ Server health check passed: {server_name} v{version}")
            log.info(f"   📊 {companies_available} companies available")
        else:
            log.warning(f"⚠️ Server health check shows degraded status: {server_name} v{version}")
    ...
    for i, query_spec in enumerate(query_plan.queries):
        ...
        while not query_successful and attempt_count < max_attempts:
            try:
                ...
                # Updated to use the new simplified server API
...
## ℹ️ **Technical Notes:**
- **Enhanced Server Compatibility**: Works with both local and Cloudflare MCP servers
- **Automatic Health Checks**: Server status verified on connection
...
async def on_mcp_connect(connection, session):
    ...
    try:
        ...
        try:
            ...
            # Perform enhanced health check using the new server API
            try:
                health_result = await check_server_health()
                if health_result.get("status") == "healthy":
                    server_name = health_result.get("server_name", connection.name)
                    version = health_result.get("version", "Unknown")
                    companies = health_result.get("companies_available", 0)
                    
                    await cl.Message(
                        content=f"✅ **Connected to {server_name}** (v{version})\n\n📊 **Server Status**: Healthy\n🏢 **Companies Available**: {companies}\n🎯 **Ready for enhanced financial analysis!**",
                        author="System"
                    ).send()
                else:
                    await cl.Message(
                        content=f"⚠️ **Connected to PSX Financial Server** (`{connection.name}`)\n\n**Status**: {health_result.get('status', 'unknown')}\n🎯 Ready for analysis (with potential limitations)",
                        author="System"
                    ).send()
            except Exception as enhanced_health_error:
                log.warning(f"⚠️ Enhanced health check failed, falling back: {enhanced_health_error}")
                await cl.Message(
            ...
            await cl.Message(
                content=f"⚠️ **Connected to PSX Financial Server** (`{connection.name}`)\n\n**Note**: Connection timeout during health check\n🎯 Ready for analysis",
                author="System"
            ).send()
...
if __name__ == "__main__":
    ...
    # Run chainlit with proper host and port for Render
    cmd = [
        sys.executable, "-m", "chainlit", "run", __file__, 
        "--host", "0.0.0.0", 
        "--port", port,
        "--headless"  # Run without browser for production
    ]
